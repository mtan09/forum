...
- **Single main feed** of user posts and news articles, with hashtags as the organizing layer (author-picked on posts, auto-extracted on articles); 7 general topics live on silently as background metadata
  - Articles are discovered from 58 real outlets across the spectrum through a rights-aware backend — see [`../forum-api`](../forum-api). Publisher pages are not scraped; restricted and unverified outlets remain useful as headline/link metadata
  ...
  - **Hot topics**: a pinned row of compact title chips above the feed, generated by clustering headline entities and event terms. Summary screens compare attributed left/center/right headlines and an original coverage note rather than copied article paragraphs
...
- **Source pages** — tap a news outlet's neutral initial mark for its detail page: rating, average article score with spread, content-type mix, and recent coverage
- **Profiles**
...
- **Rights-aware article media** — publisher images are hidden unless the API records an approved image mode. Remote-only images bypass disk caching, licensed-cache images may use the normal cache, and metadata-only stories render an intentional purple news-link design instead of a broken or empty photo frame. Publisher favicons are not fetched
...
- **forumAI**: a guided three-perspective workspace that streams Left / Center / Right readings and searches current story metadata. It treats restricted-source headlines as discovery context only; detailed grounding is reserved for text whose policy explicitly permits AI use
...
| An article has no publisher image | Expected for metadata-only sources. The app uses branded purple story artwork; images appear only when the backend records an approved image mode |
