...
- **Single main feed** of user posts and news articles, with hashtags as the organizing layer (author-picked on posts, auto-extracted on articles); 7 general topics live on silently as background metadata
  - Articles are discovered from 58 real outlets across the spectrum through a provenance-aware backend — see [`../forum-api`](../forum-api). Eligible feed/page text is analyzed transiently for scoring, clustering, search, and structured evidence; raw publisher bodies are not stored or shown
  ...
  - **Hot topics**: a pinned row of compact title chips above the feed, generated from headlines plus transiently derived entities, events, and evidence. Summary screens compare attributed left/center/right coverage in original language rather than copied article paragraphs
...
- **Resilient article media** — the backend restores publisher/feed image discovery, rejects malformed article-URL-as-image values, and prefers bounded 640px/1280px WebP variants in R2. Managed variants use the normal device cache; a failed R2 transform falls back to a remote, no-disk-cache preview. Summary carousels label every image with its publisher and tap through to the original article
...
- **forumAI**: a guided three-perspective workspace that streams Left / Center / Right readings and searches the current article corpus. It retrieves attributed headlines, links, dates, entities, timelines, claims, and agreements/disagreements from compact structured evidence—not stored publisher bodies
...
lib/article-media.ts  policy enforcement, cache mode, and URL validation for article images
...
| An article has no publisher image | The source may not provide one, the URL may be malformed/dead, or both managed and remote delivery may have failed. The app uses branded purple story artwork instead of leaving a broken frame |
